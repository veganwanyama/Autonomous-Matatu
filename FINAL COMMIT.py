...
# ---- SCOREBOARD (persistent high scores saved next to this script) ----
SCORE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "matatu_highscores.json")
MAX_SCORES = 5  # how many entries to keep on the board


def load_high_scores():
    """Read saved high scores from disk. Returns an empty list if none exist yet."""
    if os.path.exists(SCORE_FILE):
        try:
            with open(SCORE_FILE, "r") as f:
                data = json.load(f)
                if isinstance(data, list):
                    return sorted(data, reverse=True)[:MAX_SCORES]
...
def save_high_scores(scores):
    """Write the current high score list to disk."""
    try:
        with open(SCORE_FILE, "w") as f:
            json.dump(scores, f)
    except OSError:
        pass  # if we can't write to disk, just skip saving silently


def add_high_score(scores, new_score):
    """Insert a new score, keep the list sorted and capped at MAX_SCORES.
    Returns (updated_list, is_new_best)."""
    scores = scores + [new_score]
    scores.sort(reverse=True)
    scores = scores[:MAX_SCORES]
    is_new_best = (scores[0] == new_score)
...
def main():
    ...
    def fresh_game():
        return {
            ...
            "recorded": False,    # whether this game's score has been saved to the board yet
            "new_best": False,    # whether this game's score made a new #1 high score
        }

    g = fresh_game()
    running = True
    high_scores = load_high_scores()  # persists across restarts for the whole session
    ...
    while running:
        ...
        if not g["dead"]:
            ...
            # save this run's score to the scoreboard exactly once, right when it ends
            if g["dead"] and not g["recorded"]:
                g["recorded"] = True
                high_scores, g["new_best"] = add_high_score(high_scores, g["score"])
        ...
        best_so_far = high_scores[0] if high_scores else 0
        screen.blit(small_font.render(f"Best: {best_so_far}", True, GOLD), (10, 35))
        screen.blit(small_font.render("BRAKE [SPACE]", True, (200, 200, 200)), (10, 57))
        ...
        if g["dead"]:
            ...
            if g["new_best"]:
                new_best_txt = small_font.render("NEW HIGH SCORE!", True, GOLD)
                screen.blit(new_best_txt, new_best_txt.get_rect(center=(WIDTH//2, HEIGHT//2 + 40)))

            # scoreboard - top scores for the session, listed underneath
            board_top = HEIGHT//2 + 65
            board_title = small_font.render("TOP SCORES", True, GOLD)
            screen.blit(board_title, board_title.get_rect(center=(WIDTH//2, board_top)))

            for i, s in enumerate(high_scores):
                line = small_font.render(f"{i+1}. {s}", True, WHITE)
                screen.blit(line, line.get_rect(center=(WIDTH//2, board_top + 22 + i*20)))

            restart_y = board_top + 22 + len(high_scores)*20 + 20
            screen.blit(
                small_font.render("R to restart  |  ESC to quit", True, WHITE),
                small_font.render("R to restart  |  ESC to quit", True, WHITE).get_rect(center=(WIDTH//2, restart_y))
            )
